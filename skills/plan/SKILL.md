...
Use `templates/implementation-plan.md.tmpl`. Break the feature into independently completable tasks, each with:
- files to be created/modified
- a description concrete enough that a cold-start agent with zero conversation history (i.e. Cursor, opening only this file) could execute it correctly
...
## Step 6 — Write the Cursor handoff file

Write `.cursor/<slug>-apply.md` from `templates/handoff/cursor-instructions.md.tmpl`, substituting the real paths to `implementation-plan.md`, `constitution/architecture.md`, `constitution/conventions.md`, this feature's `data-model.md`, and the branch name. This is the self-contained file a cold-start Cursor session opens — it must need no other context.

## Step 7 — Report back

Summarize the task list, the trade-off questions asked and answered, the branch created (if any), and point the user at `/rsw:apply` as the next step — noting it can be run from Claude Code directly, or handed to Cursor by opening `implementation-plan.md` there.
...
- Re-run the `.cursor/<slug>-apply.md` write whenever `implementation-plan.md` is rewritten here, so a cold-start Cursor session always points at the current plan, not a stale one.
